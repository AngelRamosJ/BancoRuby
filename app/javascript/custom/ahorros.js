var mesesMax, mesesMin, tamCadMaxDecimales, tamCadMaxInteres;
  ...
  $.validator.addMethod('Numero_Interes', function(value, element) {
    if (value !== '') {
      return new RegExp("^[0-9]{1,7}\.[0-9]{1,2}$").test(value);
    } else {
      return false;
  ...
  $.validator.addMethod('Numero_Descuento', function(value, element) {
    if (value !== '') {
      return new RegExp("^[0-9]{1,7}\.[0-9]{2}$").test(value);
    } else {
      return false;
    }
...
$(document).ready(function(){
  ...
  $('#formulario_ahorros_editar').validate({
    rules: {
    'ahorro[clabe]': {
        required: true,
        rangelength: [18, 18],
