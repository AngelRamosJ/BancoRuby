class PruebasController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @prueba.save
        format.html { redirect_to prueba_url(@prueba), notice: "Prueba was successfully created." }
  ...
  def update
    respond_to do |format|
      if @prueba.update(prueba_params)
        format.html { redirect_to prueba_url(@prueba), notice: "Prueba was successfully updated." }
  ...
  private
    ...
    def prueba_params
      params.require(:prueba).permit(:nombre, :descripcion)
    end
end
